...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        robotX = pose.pose.position.x - origin.position.x
        robotY = pose.pose.position.y - origin.position.y
        rangeCount = len(scan.ranges)
        rangeData = scan.ranges    
        minAngle = scan.angle_min
        maxAngle = scan.angle_max
        increment = scan.angle_increment

        for i in range(rangeCount):
            currentRange = rangeData[i]
            # (values <= range_min or >= range_max should be discarded)
            if currentRange > scan.range_min and currentRange < scan.range_max:
                #   1. Convert the laser scan ranges and bearings to coordinates in the laser frame
                bearing = minAngle +  i * increment
                scanX = currentRange * cos(minAngle +  i * increment + robot_yaw)
                scanY = currentRange * sin(minAngle +  i * increment + robot_yaw)
                #   2. Convert the coordinates to the map frame
                mapX = scanX + robotX
                mapY = scanY + robotY
                #   3. Convert the coordinates to map indices (Hint: Use int(X) (Links to an external site.) to convert from float to int, do not use round(X) (Links to an external site.)).
                mapIndices = [int(mapX / resolution), int(mapY / resolution)]
